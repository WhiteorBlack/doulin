...
import com.luck.picture.lib.utils.AttachmentStore;
...
public class CaptureVideoActivity extends UI implements SurfaceHolder.Callback {
    ...
    private int videoTime; //录制时间 单位：秒
    private static int maxTime = 10; //录制最大时间 单位秒
    ...
    public static void start(Activity activity, String videoFilePath, int captureCode, int totalTime) {
        maxTime = totalTime;
        Intent intent = new Intent();
        intent.setClass(activity, CaptureVideoActivity.class);
        intent.putExtra(EXTRA_DATA_FILE_NAME, videoFilePath);
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            videoTime--;
            recordingTimeTextView.setText(DateUtils.timeParse(videoTime * 1000));
    ...
    private void findViews() {
        ...
        recordingTimeTextView.setText(DateUtils.timeParse(maxTime*1000));
    }
    ...
    private void tooShortAlert() {
        ToastManage.s(this, getResources().getString(R.string.video_record_short));
        cancelRecord();
    }
    ...
    private void cancelRecord() {
        ...
        recordingTimeTextView.setText(DateUtils.timeParse(maxTime));
        shutdownCamera();
        initCamera();
        startPreview();
        checkMultiCamera();
